...
function versConnexion(message: string): NextResponse {
  return NextResponse.redirect(
    new URL(`/connexion?erreur=${encodeURIComponent(message)}`, base())
  );
}
...
export async function GET(requete: NextRequest) {
  if (!googleEstConfigure()) {
    return versConnexion("La connexion Google n'est pas configurée sur cette instance.");
  }
  ...
  try {
    identite = await echangerCodeContreIdentite(code, verifieur, nonce);
  } catch {
  ...
  if (utilisateur) {
    ...
    const reponse = NextResponse.redirect(
      new URL(espaceParDefaut(utilisateur.role), base())
    );
  ...
  const reponse = NextResponse.redirect(new URL("/inscription/google", base()));
  effacerCookiesPoignee(reponse);
